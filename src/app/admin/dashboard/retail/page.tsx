...
const RETAIL_KEYWORDS = [
  'retail', 'sales', 'transaction', 'channel', 'store', 'online',
  'customer', 'consumer', 'e-commerce', 'ecommerce', 'footfall',
  'point_of_sale', 'pos', 'revenue', 'margin',
];
...
function matchesRetailKeywords(cacheKey: string): boolean {
...
export default function RetailDashboardPage() {
  ...
  const filteredAggregations = data?.cached_aggregations.filter((agg) =>
    matchesRetailKeywords(agg.cache_key)
  ) ?? [];
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-[#1B2A4A]">Retail Pulse</h1>
        <p className="text-sm text-gray-500 mt-1">
          Retail performance indicators including sales channels, customer trends, transaction patterns, and store vs online dynamics.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div>
          {filteredAggregations.length === 0 && (
            <p className="text-xs text-amber-600 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 mb-4">
              No retail-specific questions matched. Showing all available aggregations.
            </p>
          )}
        ...
        <div className="text-center text-gray-500 py-12">
          No retail data available yet. Responses will appear here once surveys are submitted.
        </div>
      )}
    </div>
  );
