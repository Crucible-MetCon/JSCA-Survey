...
const METALS_KEYWORDS = [
  'metal', 'gold', 'silver', 'platinum', 'alloy', 'karat', 'base_metal',
  'base-metal', 'palladium', 'rhodium', 'copper', 'brass', 'steel',
];
...
function matchesMetalsKeywords(cacheKey: string): boolean {
  const lower = cacheKey.toLowerCase();
  return METALS_KEYWORDS.some((kw) => lower.includes(kw));
}

export default function MetalsDashboardPage() {
  ...
  const filteredAggregations = data?.cached_aggregations.filter((agg) =>
    matchesMetalsKeywords(agg.cache_key)
  ) ?? [];

  // If no keyword matches, show all aggregations so the page is still useful
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-[#1B2A4A]">Metals &amp; Mix</h1>
        <p className="text-sm text-gray-500 mt-1">
          Metal usage patterns, alloy preferences, karat breakdowns, and precious vs base metal trends across the industry.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div>
          {filteredAggregations.length === 0 && (
            <p className="text-xs text-amber-600 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 mb-4">
              No metal-specific questions matched. Showing all available aggregations.
            </p>
          )}
        ...
        <div className="text-center text-gray-500 py-12">
          No metals data available yet. Responses will appear here once surveys are submitted.
        </div>
      )}
    </div>
  );
